...
class node {
	...
	public:
		...
		void setxpyp(int xi, int yi) {
			xp = xi;
			yp = yi;
			d = ((x - xi) > 0) + 2*((y - yi) < 0) + 3*((x - xi) < 0);	// set direction to point at parent
		}
		...
		void calch(int xf, int yf) {
			h = abs(xf - x) + abs(yf - y);	// 
			int dir = ((xf - x) > 0) + 2*((yf - y) < 0) + 3*((xf - x) < 0);	// direction to next node
			dir = (dir - d + 4)%4;
...
void print(node n[SIZE][SIZE]) {
	...
	for (int i = SIZE - 1; i >= 0; i--) {	// y-coords
		...
		cout << endl << " |\t";
		for (int j = 0; j < SIZE; j++) {
			cout << n[j][i].getxp() << "," << n[j][i].getyp() << "\t";	// parent coords
		}
		cout << endl << " |\t";
		for (int j = 0; j < SIZE; j++) {
...
void main() {
	...
	print(n);
	cout << n[curx][cury].getx() << "," << n[curx][cury].gety() << endl;
}
